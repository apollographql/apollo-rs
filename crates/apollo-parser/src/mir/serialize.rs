...
fn serialize_arguments_definition(
    state: &mut State,
    arguments: &[Harc<Ranged<InputValueDefinition>>],
) -> fmt::Result {
...
fn serialize_schema(
    state: &mut State,
    directives: &[Harc<Ranged<Directive>>],
...
fn serialize_object_type_like(
    ...
    directives: &[Harc<Ranged<Directive>>],
    fields: &[Harc<Ranged<FieldDefinition>>],
) -> Result<(), fmt::Error> {
    state.write(name)?;
...
fn serialize_directives(state: &mut State, directives: &[Harc<Ranged<Directive>>]) -> fmt::Result {
...
fn serialize_arguments(state: &mut State, arguments: &[(BowString, Value)]) -> fmt::Result {
    if !arguments.is_empty() {
        state.on_single_line(|state| {
...
fn serialize_description(state: &mut State, description: &Option<BowString>) -> fmt::Result {
    ...
    ($($ty: ident)+) => {
        $(
            ...
            impl $ty {
                ...
                /// and implements the [`Display`] and [`ToString`] traits by writing GraphQL syntax.
            ...
            impl fmt::Display for Serialize<'_, $ty> {
                fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
